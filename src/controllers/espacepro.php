...
class EspaceproController extends bd
{
	...
	function saveligneficher()
	{
		...
		if ($data['id_module'] > 0) {
			...
			$table->save($data, 'id');
			$id = $table->getId();
			$arraydata=json_decode($data['dataa']);
			if (count($arraydata) > 0) {
				foreach ($arraydata as $f) {
					foreach ($f as $k=>$ff) {						
					$nom = str_replace(" ","",$k);
					//echo $nom ;
					if (isset($this->chapitre[$nom])) {
						$array = array('date' => date('Y-m-d'), 'id_produit' => $data['id_produit'],'id_module' => $data['id_module'], 'id_chapitre' => $this->chapitre[$nom]->id, 'fichier' => $ff, "id_fichier" => $id);
	...
	function saveficher()
	{
		...
		if ($book) {
			...
			$data['fichier']="media/files/".$data['fichier'];
			$sql = "select * from #__chapitre where id_module=" . intval($data['id_module']);
			$this->setQuery($sql);
			$this->chapitre = $this->getLignes('code');
			...
			if ($data['id_module'] > 0) {
				...
				if (count($files) > 0) {
					foreach ($files as $f) {
						...
						$nom = substr($f, 0, $nb);
						if (isset($this->chapitre[$nom])) {
							$array = array('date' => date('Y-m-d'), 'id_module' => $data['id_module'], 'id_produit' => $data['id_produit'], 'id_chapitre' => $this->chapitre[$nom]->id, 'fichier' => $f, "id_fichier" => $id);
							$table = new TableLigneFichier();
							$table->save($array, 'id');
						}
					}
...
function saveSpecialite(){
	...
	$data=$_REQUEST;
	$lang = $session->language;
	$msg=$lang->text('ECHEC_SAVE');$type='error';
	$book=true; 
	
		if(isset($data['id']) ){
			if(is_numeric($data['id'])){ 
				$sql="select * from #__specialite where id=".intval($data['id']);
				$this->setQuery($sql);
				$item=$this->getLine();
				if(!$item) $book=false;
					$msg = $lang->text('VERIF_DONNEE');
				}else{
				$book=false;	
			}
		...
		if($book){
			$msg=$lang->text('SUCCESSFUL_RECORDING');$type='success'; 
			$table =new TableSpecialite();//die(print_r($data));
			$table->save($data,'id');
		}
Router::page(Router::base().'admin-specialite',$msg,$type);
}
...
function deleteSpecialite(){
	...
	$lang=$session->language;
	$data=$_REQUEST;
	$msg=$lang->text('ECHEC_DELETE');$type="error";
	
	if($data['id']){
		
		$sql="select * from #__specialite where id =".intval($data['id']);
		$this->setQuery($sql);
		$specialite=$this->getLine();
		$msg=$lang->text('INTROUVABLE');
		if($specialite){
				 $msg =$lang->text('SUCESS_SUPPRIMER');
		 $type="success";
		 
		 $sql="delete from #__specialite where id =".intval($data['id']);
		 $this->setQuery($sql);
		 $this->execute();
