 ...
 class Router{
	...
	public function __Construct(){
		...
		if($this->multilang && count($tab)>=1){
			...
			//die(array_search($tab[0],$file) .'**'.$tab[0]);

				/*if($key=array_search($tab[0],$file)!==false){}else if(count($tab)==1){
				...
				switch($tab[0]){
					...
					case 'fr':$this->current_lang=0;break;

					case 'en':$this->current_lang=1;break;
